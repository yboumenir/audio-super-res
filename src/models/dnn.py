    ...
    def create_model(self, n_dim, r):
        ...
        with tf.name_scope('generator'):
            ...
            for l in range(L):
                ...
                # Determine output units based on layer index
                if l == L - 1:
                    out_units = n_dim
                elif l == 0:
                    out_units = n_dim // 6
                ...
                # Determine input shape based on layer index
                in_shape = n_dim // (6 if l == 0 else 2)

                # Create the Dense layer
                x = tf.keras.layers.Dense(units=out_units,
                                          kernel_initializer=tf.keras.initializers.RandomNormal(stddev=1e-7))(x)
                x = tf.keras.layers.BatchNormalization()(x)
                x = tf.keras.layers.LeakyReLU(0.2)(x)

                # Add residual connection if shapes match
                if tf.reduce_all(tf.equal(x_shape, tf.shape(input=x_start))):
                    x = tf.add(x, x_start)
