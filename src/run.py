...
def make_parser():
    ...
    model_choices = ('audiounet', 'audiotfilm', 'dnn', 'spline')
    # train

    train_parser = subparsers.add_parser('train')
    train_parser.set_defaults(func=train)

    train_parser.add_argument('--model', default='audiotfilm', choices=model_choices, help='model to train')
    ...
    train_parser.add_argument('--train', required=True, help='path to h5 archive of training patches')
    train_parser.add_argument('--val', required=True, help='path to h5 archive of validation set patches')
    train_parser.add_argument('-e', '--epochs', type=int, default=100, help='number of epochs to train')
    train_parser.add_argument('--batch-size', type=int, default=128, help='training batch size')
    train_parser.add_argument('--logname', default='tmp-run', help='folder where logs will be stored')
    ...
    eval_parser.add_argument('--logname', required=True, help='path to training checkpoint')
    eval_parser.add_argument('--out-label', default='', help='append label to output samples')
    eval_parser.add_argument('--wav-file-list', help='list of audio files for evaluation')
    eval_parser.add_argument('--r', help='upscaling factor', default=4, type=int)
    eval_parser.add_argument('--sr', help='high-res sampling rate', type=int, default=16000)
    eval_parser.add_argument('--grocery', default='false', choices=('true', 'false'))
    eval_parser.add_argument('--model', default='audiounet', choices=model_choices, help='model to train')
    ...
    simple_run =  subparsers.add_parser('run')
    simple_run.set_defaults(func=run)
    simple_run.add_argument('--file', required=True, help='file to up sample')
    simple_run.add_argument('--logname', required=True, help='path to training checkpoint')
    simple_run.add_argument('--out-label', default='', help='append label to output samples')
    # simple_run.add_argument('--wav-file-list', help='list of audio files for evaluation')
    simple_run.add_argument('--r', help='upscaling factor', default=4, type=int)
    simple_run.add_argument('--sr', help='high-res sampling rate', type=int, default=16000)
    simple_run.add_argument('--grocery', default='false', choices=('true', 'false'))
    simple_run.add_argument('--model', default='audiounet', choices=model_choices, help='model to train')
    simple_run.add_argument('--speaker', default='single', choices=('single', 'multi'),
                             help='number of speakers being trained on')
    simple_run.add_argument('--pool_size', type=int, default=4, help='size of pooling window')
    simple_run.add_argument('--strides', type=int, default=4, help='pooling stide')
    simple_run.add_argument('--patch_size', type=int, default=8192,
                             help='Size of patches over which the model operates')
...
def train(args):
    ...
    if (args.model == 'seq2seq'):
        model = models.Model2()
        model.run(X_train, Y_train, X_val, Y_val, n_epoch=args.epochs, r=args.r, speaker=args.speaker,
        ...
        # train model
        model.fit(X_train, Y_train, X_val, Y_val, n_epoch=args.epochs, r=args.r, speaker=args.speaker,
                  grocery=args.grocery, piano=args.piano, calc_full_snr=full)
...
def run(args):
    ...
    try:
        upsample_wav(file, args, model)
    except EOFError:
        print(f'WARNING: Error reading file: {file}')
